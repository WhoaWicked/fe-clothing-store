...
export default function OrderSuccessPage() {
  ...
  // State เอาไว้เช็คว่ามี session_id ไหม (Optional)
  const [isValid, setIsValid] = useState(false);

  useEffect(() => {
    if (sessionId) {
      setIsValid(true);
      // ในอนาคต: ตรงนี้สามารถยิง API ไป Backend เพื่อเช็คสถานะออเดอร์จริงๆ ได้
      // const checkStatus = async () => { ... }
    } else {
      // ถ้าไม่มี session_id (แอบเข้าผ่าน URL ตรงๆ) อาจจะดีดกลับหน้าแรก
      // router.push('/'); 
    }
  }, [sessionId, router]);

  if (!isValid && !sessionId) {
    return <div className='min-h-screen -mt-16 flex items-center justify-center'>
      <div>
        <div>
    ...
    <div className="mt-15 flex items-center justify-center">
      <div className="max-w-md w-full border border-gray-300 shadow-lg px-8 py-12">
        ...
        <div className='tracking-wide'>
          ...
          {/* แสดง Session ID (เอาไว้ Debug ดูเล่นๆ ว่าค่ามาจริงไหม) */}
